  ...
  <div class="tables-basic">
    <b-row>
      ...
      >
        ...
        size="sm"
        class="auth-btn mb-3"
        @click="ChangeSerialStoreByCartoonId()"
        variant="inverse"
        style="margin-right: 10px;"
        >تحويل عن طريق رقم الكارتون</b-button
      >
    ...
    <form class="mt" @submit.prevent="getGetSerials()">
      ...
      <b-row>
        ...
        <b-col>
            <label style="margin-bottom: 6px">نقاط البيع</label>
            <b-input-group class="mb-3">
              <b-form-select
                style="height: 37px"
                :options="clinet"
                  v-model="search.cliantName"
              ...
              <tbody>
                <tr v-for="(row, index) in allserials" v-bind:key="row.id">
                  ...
                  <td v-else>
                        ...
                        variant="inverse"
                        style="    margin-right: 10px;"
                        ><i class="fa fa-undo" aria-hidden="true"></i></b-button
                      >
                  </td>
          ...
          <b-col>
            <div class="form-group">
              ...
              <input
                ...
                name="serialInfo"
                required
                placeholder="رقم الكارتون او رقم الموديل او رقم الجهاز او رقم الكارتون  المحلي"
              />
            </div>
          </b-col>
          <b-col>
    ...
    <b-modal
      id="ChangeSerialStoreByCartoonId"
      hide-footer
      hide-header
      no-close-on-backdrop
      body-text-variant="light"
    >
      <div class="d-block text-center">
        ...
        <form class="mt" @submit.prevent="changecartoonidlistnow()">
          <b-alert class="alert-sm" variant="danger" :show="!!errorMessage">
            {{ errorMessage }}
          </b-alert>
          <b-col>
            <div class="form-group">
              ...
              <input
                class="form-control no-border"
                v-model="cartoonIdforlist"
                type="text"
                name="serialInfo"
                required
                placeholder="رقم الكارتون"
              />
            </div>
          </b-col>
          
            <b-col cols="12"
              ><b-button class="mt-3" variant="success" block @click="addidtocartoonlist()"
                >اضافة</b-button
              ></b-col
            >
          <b-col>
        <b-col cols="12">
          <ul>
            <li v-for="(row, index) in cartoonIdlist" v-bind:key="row.cartoonId">
               <span style="padding: 2px;
                ...
                font-weight: 900;
                text-align: right;
                font-size: 17px;">{{index}} - {{row.cartoonId}}</span>
            </li>
          </ul>
        </b-col>  
          ...
          <b-row>
            <b-col cols="4">
              <b-button
                ...
                variant="danger"
                block
                @click="$bvModal.hide('ChangeSerialStoreByCartoonId')"
                >غلق</b-button
              ></b-col
            >
    ...
    >
      <div class="d-block text-center">
        ...
        <form class="mt" @submit.prevent="sentserialtoclinetssnow()">
          <b-alert class="alert-sm" variant="danger" :show="!!errorMessage">
            {{ errorMessage }}
          </b-alert>
          <b-col>
          ...
          <b-col>
            ...
            <b-input-group class="mb-3">
              <b-form-select
                style="height: 37px"
                :options="clinet"
                v-model="changeformtoclinet.clinetid"
                required
              />
            </b-input-group>
          </b-col>
    ...
    >
      <div class="d-block text-center">
        ...
        <form class="mt" @submit.prevent="changeselecttosalesepoint()">
          <b-alert class="alert-sm" variant="danger" :show="!!errorMessage">
            {{ errorMessage }}
          </b-alert>
          <b-col>
          ...
          <b-row>
            <b-col cols="4">
              <b-button
                ...
                variant="danger"
                block
                @click="$bvModal.hide('sentserials3')"
                >غلق</b-button
              ></b-col
            >
...
export default {
  ...
  methods: {
    addidtocartoonlist(){
      ...
      var ids = {
        "cartoonId" : this.cartoonIdforlist
      }
      this.cartoonIdlist.push(ids);
    ...
    changeselect2() {
      ...
      this.$bvModal.show("sentserials3");
    },


  ChangeSerialStoreByCartoonId() {
      this.$bvModal.show("ChangeSerialStoreByCartoonId");
    },


       sentclinet() {
      this.$bvModal.show("sentclinet");
    },
        sentserialtoclinetssnow() {
      this.isLoading = true;
      this.axios
        .put(
          `Admin/ChangeSerialToClient?serialInfo=${this.changeform.serialInfo}&clientId=${this.changeformtoclinet.clinetid}`
        )
        .then((res) => {
          if (res) {
            this.$swal("تم!", "  تم الاضافة بنحاج", "success");
            this.$bvModal.hide("sentclinet");
            this.isLoading = false;
            this.getGetSerials();
            this.changeform = {};
          }
    ...
    changeselecttosalesepoint() {
      this.$swal({
        title: "هل انت متأكد?",
        text: "هل انت متأكد من نقل الى نقطة البيع",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          ...
          this.axios
            .put(
              `Admin/ChangeToClientByIds?clientId=${this.changeformtoclinet.clinetid}`,
              this.listid
            )
            .then((res) => {
              if (res) {
                ...
                this.selected = [];
                this.listid = [];
                this.$bvModal.hide("sentserials3");
                this.isLoading = false;
              }
            })
    ...
    restore(id) {
      this.$swal({
        title: "هل انت متأكد?",
        text: "هل انت متأكد من استرجاع المباع",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
          ...
          this.axios
            .put(
              `Admin/RestoreSolidSeriel?serialId=${id}`,
              this.listid
            )
            .then((res) => {
              if (res) {
                this.$swal("تم!", "تم الاسترجاع بنحاج", "success");
                this.getGetSerials();
                this.selectAll = false;
                this.selected = [];
                this.listid = [];
    ...
    deletselect() {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          this.axios
            .put("Admin/DeletSerialStoreByIds", this.listid)
            .then((res) => {
              if (res) {
    ...
    addserialsnow() {
      ...
      this.axios
        .post("Admin/AddXlsxSerailFile", formData, {
          headers: {
    ...
    changecartoonidlistnow() {
      if(this.cartoonIdlist.length == 0){

        this.$swal("انتاه!", "يرجى اضافة رقم كارتو واحد كحد ادنى", "info");
      }
      this.isLoading = true;
      this.axios
        .put(
          `Admin/ChangeSerialStoreByCartoonId?storeId=${this.changeform.StoreId}`, this.cartoonIdlist
        )
        .then((res) => {
          if (res) {
            this.$swal("تم!", "  تم الاضافة بنحاج", "success");
            this.$bvModal.hide("ChangeSerialStoreByCartoonId");
            this.isLoading = false;
            this.getGetSerials();
            this.changeform = {};
          }
    ...
    getGetSerials() {
      ...
      this.axios
        .get(
          `Admin/GetSerial?skip=0&take=100&fileName=${this.search.fileName}&storeId=${this.search.storeId}&date=${this.search.date}&serialInfo=${this.search.serialInfo}&cliantName=${this.search.cliantName}&localID=${this.search.localID}`
        )
        .then((res) => {
    ...
    getMore: function() {
      ...
      this.axios
        .get(
          `Admin/GetSerial?skip=${tempPage - 1}&take=100&fileName=${
            this.search.fileName
          }&storeId=${this.search.storeId}&date=${
            this.search.date
          }&serialInfo=${this.search.serialInfo}&cliantName=${this.search.cliantName}&localID=${this.search.localID}`
        )
      ...
      this.$swal({
        ...
        .then((result) => {
          if (result.isConfirmed) {
            ...
            this.axios
              .get(
                `Admin/DeleteSearchSerial?fileName=${this.search.fileName}&storeId=${this.search.storeId}&date=${this.search.date}&serialInfo=${this.search.serialInfo}`
              )
              .then((res) => {
                if (res) {
                  this.allserials = [];
